...
func TestEchoAgentService_AskPermission_ThisTime(t *testing.T) {
	...
	// Verify permission request fields.
	...
	// Verify the reply includes the decision result.
...
// TestEchoAgentService_AskPermission_ChatID verifies: handler receives the same chatID
// as the request.
func TestEchoAgentService_AskPermission_ChatID(t *testing.T) {
	...
	_, err := svc.Prompt(context.Background(), chatID, domain.PromptInput{Text: "[ask]"})
	require.NoError(t, err)
